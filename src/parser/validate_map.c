/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   validate_map.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mattcarniel <mattcarniel@student.42.fr>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/12 14:54:41 by mattcarniel       #+#    #+#             */
/*   Updated: 2026/03/18 16:39:37 by mattcarniel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdbool.h>
#include <stddef.h>

#include "parser_internal.h"

#include <stdio.h>

static bool	is_tile_player(t_assets *a, size_t pos)
{
	return (a->tiles[a->map.data[pos]].flags & TILE_F_PLAYER);
}

static bool	is_tile_valid(t_assets *a, size_t pos)
{
	size_t	x;
	size_t	y;

	x = pos % a->map.width;
	y = pos / a->map.width;
	return (!(a->tiles[a->map.data[pos]].flags & TILE_F_WALKABLE) ||
		(x != 0 && x != a->map.width - 1 && y != 0 && y != a->map.height - 1
		&& !(a->tiles[a->map.data[pos - 1]].flags & TILE_F_VOID)
		&& !(a->tiles[a->map.data[pos + 1]].flags & TILE_F_VOID)
		&& !(a->tiles[a->map.data[pos - a->map.width]].flags & TILE_F_VOID)
		&& !(a->tiles[a->map.data[pos + a->map.width]].flags & TILE_F_VOID)));
}

int	validate_map(t_assets *a)
{
	size_t	i;
	bool	player_found;

	i = 0;
	player_found = false;
	while (i < a->map.width * a->map.height)
	{
		if (is_tile_player(a, i))
		{
			if (player_found)
				return (dprintf(2, "Map: Multiple players found\n"), 1);
			a->map.player_pos = i;
			player_found = true;
		}
		if (!is_tile_valid(a, i))
			return (dprintf(2, "Map: tile not properly surrounded found at x=%zu, y=%zu\n", i % a->map.width, i / a->map.width), 1);
		i++;
	}
	if (!player_found)
		return (dprintf(2, "Map: No players found\n"), 1);
	return (0);
}
